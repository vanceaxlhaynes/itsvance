export default async function handler(req, res) {
  ...
  const databases = {
    ...
    landing:   process.env.NOTION_DB_LANDING,
    lenses:    process.env.NOTION_DB_LENSES
  };

  const dbId = databases[type];
  if (!dbId) {
    return res.status(400).json({ error: 'Invalid type. Use: writings, movies, music, birthdays, landing, lenses' });
  }
  ...
  try {
    const response = await fetch(`https://api.notion.com/v1/databases/${dbId}/query`, {
      ...
      body: JSON.stringify({
        ...
        sorts: [
          { property: 'Order', direction: type === 'lenses' ? 'descending' : 'ascending' }
        ]
      })
    });
    ...
    const items = data.results.map(page => {
      ...
      const get = (prop, fallback = '') => {
        ...
        if (val.type === 'files' && val.files?.length) {
          const f = val.files[0];
          return f.type === 'file' ? f.file?.url || fallback : f.external?.url || fallback;
        }
        return fallback;
      };

      // Get ALL file URLs from a files property (for multi-photo entries)
      const getFiles = (prop) => {
        const val = p[prop];
        if (!val || val.type !== 'files' || !val.files?.length) return [];
      ...
      if (type === 'writings') {
        ...
        item.body = get('Body');
        item.sign = get('Sign', '- Vance');
        item.buttonText = get('Button Text', 'come in');
      } else if (type === 'lenses') {
        item.photoUrl = get('Photo URL');
        item.caption = get('Caption');
      }
